    ...
    createFileList = () => {
        ...
        let markup = this.state.fileList.files.map((file, index) => {
            ...
            if (file.file.type.indexOf('video') > -1) {
                media = <Typography variant="h3" gutterBottom>{originFile.name}</Typography>;
    ...
    render() {
        ...
        if (this.props.startButton) {
            if (typeof this.props.startButton ==="string" || typeof this.props.startButton ==="boolean" ) startButton = <label>
                <button disabled={this.state.isUploading} className="btn start" onClick={this.startUpload}>{this.props.startButton && "start"}
                </button>
            </label>;
        ...
        if (this.props.cancelButton) {
            if (typeof this.props.cancelButton ===  "string" || typeof this.props.cancelButton ===  "boolean")cancelButton = <label>
                <button disabled={!this.state.isUploading}  onClick={this.cancelUpload}>{this.props.cancelButton && "cancel"}
                </button>
            </label>;
        ...
        if (this.props.pauseButton) {
            if (typeof this.props.pauseButton ===  "string" || typeof this.props.pauseButton ===  "boolean") pauseButton = <label>
                <button disabled={!this.state.isUploading}  onClick={this.pauseUpload}>{this.props.pauseButton && "pause"}
                </button>
            </label>;
        ...
        return (
            <div id={this.props.dropTargetID} ref={node => this.dropZone = node}>
                {}
                <label style={{disabled: this.props.disableInput ? true : false}}
                  className={this.props.disableInput ? 'btn file-upload disabled' : 'btn file-upload'}>{textLabel}
                    <input
                ...
                <div className="progress" style={{display: this.state.progressBar === 0 ? "none" : "block", padding: "20px"}}>
